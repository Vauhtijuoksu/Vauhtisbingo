...
$(document).ready(function(){
    ...
    if (username){
        seed_spot = stringToNumber(username)
...
var seed_spot = 0
function random() {
    var x = Math.sin(seed + seed_spot) * 1000.0;
    seed += 1
...
function generate() {
    ...
    for (var row = 0; row < 5; row++) {
        ...
        for (var cell = 0; cell < 5; cell++) {
            ...
            html += "<div class='board-cell r-" + row + " c-" + cell + " " + unlocked + "'><div class='cell'><div style='height: 100%; width: 100%'>"
            let rnum = random()
            while (rnum === 1) {
                rnum = random()
            }
            ...
            if (row === 2 && cell === 2) {
                if (mid){
                    ...
                    mrnum = Math.floor(mid.length * mrnum)
                    cell_text = mid[mrnum]
                    cell_hover = mid[mrnum]
                }
            }
          html += "<div style='display: flex; flex-direction: column; justify-content: center; align-items: center; height: 100%'><div style='flex-grow: 1; max-height: 80%; max-width: 90%;'><img style='height: 100%; width: 100%; object-fit: contain' src='./fish/trimmed/" + cell_text + "'/></div><div>" + cell_hover.split(".")[0].replaceAll("_", " ") + "</div></div>"
            html += "</div></div>"
            html += "<div class='hover'><div>" + cell_hover.split(".")[0].replaceAll("_", " ") + "</div></div></div>"
        }
        html += "</div>"
    }
    $("#board").html(html)
